...
if ($_SERVER['REQUEST_METHOD']=="POST") {
    ...
    if (empty($carnet)) {
        $errores['carnet']=  "Debes Ingresar tu correo electrónico";
    }
if (empty($password)) {
...
if (empty($errores)) {
    try {
        ...
        $sql="SELECT * FROM pf WHERE carnet=:carnet ";
        ...
        //los valores que estan despues del igual en los corchetes son de la base de datos
                ...
                $_SESSION['codigoPF']=$user['codigoPF'];
                $_SESSION['generos']=$user['genero'];
                $_SESSION['federacion']=$user['federacion'];
                $_SESSION['carnet']=$user['carnet'];
                $_SESSION['ciudad']=$user['ciudad'];
                $_SESSION['numeroPF']=$user['numeroPF'];
                $login=true;
            }
        }
        ...
        if ($login) {
            ...
            echo "El carnet y contraseña no estan registrado";
        }
