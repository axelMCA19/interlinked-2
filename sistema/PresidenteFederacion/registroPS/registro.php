...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $codigoPS = isset($_POST['codigoPS']) ? $_POST['codigoPS'] : null;
    ...
    $numeroPS = isset($_POST['numeroPS']) ? $_POST['numeroPS'] : null;
    ...
    if (empty($codigoPS)) {
        $errores['codigoPS'] = "Debes ingresar tu código de afiliado";
    }
    if (empty($genero)) {
    ...
    if (empty($errores)) {
        try {
            ...
            $sql = "SELECT * FROM codigos_presidentes WHERE codigo = :codigo AND estado = 0";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':codigo' => $codigoPS]);
            ...
            if ($codigoDisponible) {
                ...
                $sql_user_exists = "SELECT * FROM presidentes WHERE carnet = :carnet";
                ...
                if ($userExists) {
                    ...
                    $sql_insert = "INSERT INTO presidentes (nombres, apellidos, codigoPS, password, genero, federaciones, carnet, sindicato, numeroPS) 
                        VALUES (:nombres, :apellidos,  :codigoPS, :password, :genero, :federaciones,  :carnet, :sindicato, :numeroPS)";
                    ...
                    $sql_update = "UPDATE codigos_presidentes SET estado = 1 WHERE codigo = :codigo";
                    $stmt_update = $pdo->prepare($sql_update);
                    $stmt_update->execute([':codigo' => $codigoPS]);
...
<html lang="en">
<head>
    <title>Registro presidente</title>
    <!-- Required meta tags -->
    <meta charset="utf-8" />
    ...
    <link rel="stylesheet" href="registroPS.css">
                ...
                <div class="col-md-8 col-lg-8">
                    ...
                    <?php if (isset($successes) && $successes) { ?>
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            ...
                            <strong>¡Presidente registrado correctamente!</strong> Felicidades has registrado a un nuevo presidente a tu <?php echo $_SESSION['federacion'];?>! clic 
                            <a
                                name=""
                                id=""
                                class="btn btn-primary"
                                href="../ListasPS/index.php"
                                role="button"
                                >aqui</a
                            >
                            para poder ver tus presidentes registrados
                        </div>
                    ...
                    <div class="card">
                        <div class="card-header"><h3 style="text-align: center;"><strong>Formulario de registro de presidente de sindicato</strong></h3></div>
                        <p class="parrafo"><strong>Bienvenido sr presidente <strong><?php echo $_SESSION['usuario_nombre'];?> </strong>aqui podras registrar a tus presidentes</strong></p>
                        <div class="card-body">
                            <form action="registro.php" id="formularios" method="post">
                                ...
                                <div class="row mb-3">
                                    <div class="col">
                                        <label for="" class="form-label"><strong>Numero de presidente</strong></label>
                                        <input type="Text" class="form-control" name="numeroPS" id="numeroPS" required placeholder="N° de presidente" />
                                    </div>
                                    <div class="col">
                                ...
                                <div class="row mb-3">
                                    <div class="col">
                                        <label for="" class="form-label"><strong>Codigo presidente:</strong></label>
                                        <input type="text" class="form-control" name="codigoPS" id="codigoPS" aria-describedby="helpId" required placeholder="Codigo afiliado" />
                                    </div>
                                    <div class="col">
                                        <label for="" class="form-label"><strong>Sindicato:</strong></label>
                                        <input type="text" class="form-control" name="sindicato" id="sindicato" required oninput="this.value = this.value.toUpperCase()" placeholder="sindicato" />
                                    </div>
                                </div>
